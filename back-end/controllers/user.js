...
export const uploadImage = async (req, res) => {
    ...
    try {
        ...
        await req.body.photos.forEach(async (photo) => {
            await client.query('UPDATE users SET balance = balance+$1, current_storage = current_storage+$2 WHERE tg_id = $3', [photo.price.toFixed(3), photo.size, req.body.tg_id]);
            await client.query('UPDATE users SET income = income+$1 WHERE tg_id = $2', [photo.price.toFixed(3), req.body.tg_id]);
        })
        await res.json({success: true});
    } catch (err) {
        await console.log(err);
